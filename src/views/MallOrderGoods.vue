<template>
  <div>
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'" @selection-change="handleSelectionChange">>
      ...
      <el-table-column prop="goodsSn" label="商品编码"></el-table-column>
    ...
    <el-dialog title="订单信息" :visible.sync="dialogFormVisible" width="25%" >
...
export default {
  ...
  methods: {
    ...
    load() {
      this.request.get("/mallOrderGoods/page", {
        params: {
    ...
    del(id){
      this.request.delete("/mallOrderGoods/"+id).then(res => {
        if(res){
    ...
    Save() {
      this.request.post("/mallOrderGoods", this.form).then(res => {
        if (res) {
    ...
    delBatch(){
      ...
      this.request.post("/mallOrderGoods/del/batch",ids).then(res=>{
        if(res)
        {
